...
% g_sum = g_cumy(1);
% for j = 2:I*J
%     g_sum = [g_sum; g_sum(j-1)+g_cumy(j)];
% end
% 
% sav_L = reshape(ss(:,:,1)/(w*z(1)),I*J,1); % saving rate
% sav_H = reshape(ss(:,:,2)/(w*z(2)),I*J,1);
% 
% risky_share_L = reshape(share_a(:,:,1),I*J,1); % risky share
% risky_share_H = reshape(share_a(:,:,2),I*J,1);
% 
% a_L = reshape(a_final(:,:,1),I*J,1);
% a_H = reshape(a_final(:,:,2),I*J,1);
% 
% b_L = reshape(b_final(:,:,1),I*J,1);
% b_H = reshape(b_final(:,:,2),I*J,1);
% 
% g_wealth = [wealth_vec, g_cumy, g_sum, sav_L, sav_H, risky_share_L, risky_share_H, a_L, a_H, b_L, b_H];
% g_wealth = sortrows(g_wealth,1);
% 
% % % T = table(g_wealth);%,'VariableNames',{'wealth_vec','g_cumy','sav_L','sav_H','risky_share_L','risky_share_H','a_L','a_H','b_L','b_H'});
% % T = table(wealth_vec, g_cumy, g_sum, sav_L, sav_H, risky_share_L, risky_share_H, a_L, a_H, b_L, b_H);
% % filename = 'results_data.xlsx';
% % writetable(T,filename,'Sheet',1)
% 
% ind = 1;
% for i=2:I*J
%     if g_wealth(i,1) == g_wealth(i-1,1);
%         ind = [ind; ind(i-1)];
%     else
%         ind = [ind; ind(i-1)+1];
%     end
% end
% 
% g_wealth = [g_wealth, ind];
% 
% gsum = accumarray(ind,g_wealth(:,2));
% wealth_grid = unique(g_wealth(:,1));


% figure 
% bar(wealth_grid',gsum,'r'); grid on, hold on
% xlabel('Financial Wealth','Fontsize',12,'interpreter','latex')
% xlim([-0.1 20])
% % ylim([0 1.4])
% % text(0.2,0.9,['$\leftarrow Pr(a=0)=' num2str(round(da*marg_a(1,1),2)) '$'],'FontSize',14,'interpreter','latex','Color','k');
% hold off