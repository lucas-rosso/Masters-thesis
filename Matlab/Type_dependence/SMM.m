...
mu    = 0.06;  
sigma_diff    = params(3); % Gomes and Michaelides (JF, 2005)                   

% adding type dependence with imperfect diversification
varthe = params(2);
s2 = (sigma_diff*exp(-varthe.*a)).^(2);
...
%% Average std deviation risky asset
std = sqrt(s2);
avg_std = sum(marginal_a.*std)*da;
%% Wealth shares
...
m40 = (round(g_sum,2) > 0.50 & g_sum < 0.90);
...
% middle 40%
middle40 = wealth_vec'*(gcum.*m40)
middle40_share = middle40/total_wealth
...
%% Loss function
n_mom = 3; % number of moments internally calibrated
% data_moments = [0.512, 0.375]';
% th_moments = [part_rate, top1_share]';

data_moments = [0.512, 0.778,0.18]';
th_moments = [part_rate, top10_share, avg_std]';
loss = (data_moments-th_moments)'*eye(n_mom)*(data_moments-th_moments)

table = {'Part. Rate', round(part_rate,3)*100, 51.2; ...
         'Top 10% share', round(top10_share,3)*100, 77.8;...
         'Std. deviation sigma%', round(avg_std,3), 0.18}
