...
% Targeting bottom 50 wealth share
% Author: Lucas Rosso %
% Date: 05-02-2021 %
...
% z     = [0.75,1.25]; % state values (row vector)
% la    = [1/4,1/4]; % Poisson intensities

% Peter Maxted (2020), Guerrieri and Lorenzoni (2017)
% z     = [0.74,1.26]; % state values (row vector)
% la    = [0.19,0.19]; % Poisson intensities
...
% la = [ 0.9683, 0.0577]; %job creation rate 25% (monthly), unemployment rate 5.62 (avg. 1998-2019)
% z  = [0.71,1.017]; % y1 -> hall & millgrom (2008, AER), y2 -> E(y) = 1

% jesus FV
% z     = [0.72,1.015]; % state values (row vector), y1 -> Hall and Milgrom (2008), y2 -> E(y) = 1
% la    = [0.986,0.052]; % Poisson intensities la1 -> monthly job finding rate of 0.3, la2 -> unemplyment rate 5%
% --------------------------------------------------------------------------
...
fprintf('Made it !!')
save('DATA_05022021') % to avoid going through the iteration again.

run Figures_05022021.m
