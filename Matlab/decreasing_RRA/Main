...
% Main Code but with decreasing RRA
% Author: Lucas Rosso %
% Date: 14-04-2021 %
...
alpha = 0.1125;                  % "subsistence parameter" Achury et al (2012, RED)
...
% Initial guess
V0(:,:,1) = (1-s)^(-1)*(z(1) + rbPos.*bb -alpha).^(1-s)/rho; % assuming u(c) = (c-alpha)^(1 - s)/(1 - s). This is similar to value of "staying put" u(z + rb*b)/rho                                                          
V0(:,:,2) = (1-s)^(-1)*(z(2) + rbPos.*bb -alpha).^(1-s)/rho;
v         = V0;  
...
for n=1:maxit
    ...
    Vbf(I,:,:,:) = (zzz(I,:,:) + Rb(I,:,:).*bmax -alpha).^(-s); %will never be used, but impose state constraint b<=bmax just in case
                                                           % this is v'b(I,j,k)_F = u'(z(k) + rb(I) * b(I))
    % backward difference
    Vbb(2:I,:,:) = (V(2:I,:,:)-V(1:I-1,:,:))/db;
    Vbb(1,:,:) = (zzz(1,:,:) + Rb(1,:,:).*bmin -alpha).^(-s); %state constraint boundary condition: v'b(1,j,k)_B = u'(z(k) + rb(1) * b(1))

    Vbf = max(Vbf,10^(-6)); % this is to avoid dealing with too small numbers in parts where v is almost flat wrt b
    Vbb = max(Vbb,10^(-6));
    
     %consumption and savings with forward difference
     cf = Vbf.^(-1/s) +alpha; % from FOC
     sf = zzz + Rb.*bbb - cf;
