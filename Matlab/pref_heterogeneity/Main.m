...
s     = [2.5,1.5];                   % risk aversion in CRRA utility
...
% Initial guess
V0(:,:,1) = (1-s(1))^(-1)*(z(1) + rbPos.*bb).^(1-s(1))/rho; % assuming u(c) = c^(1 - s)/(1 - s). This is similar to value of "staying put" u(z + rb*b)/rho                                                          
V0(:,:,2) = (1-s(2))^(-1)*(z(2) + rbPos.*bb).^(1-s(2))/rho;
v         = V0;  
...
for n=1:maxit
    ...
    Vbf(I,:,1) = (zzz(I,:,1) + Rb(I,:,1).*bmax).^(-s(1)); %will never be used, but impose state constraint b<=bmax just in case
    Vbf(I,:,2) = (zzz(I,:,2) + Rb(I,:,2).*bmax).^(-s(2));  % this is v'b(I,j,k)_F = u'(z(k) + rb(I) * b(I))
    
    % backward difference
    Vbb(2:I,:,:) = (V(2:I,:,:)-V(1:I-1,:,:))/db;
    Vbb(1,:,1) = (zzz(1,:,1) + Rb(1,:,1).*bmin).^(-s(1)); %state constraint boundary condition: v'b(1,j,k)_B = u'(z(k) + rb(1) * b(1))
    Vbb(1,:,2) = (zzz(1,:,2) + Rb(1,:,2).*bmin).^(-s(2));
    
    Vbf = max(Vbf,10^(-6)); % this is to avoid dealing with too small numbers in parts where v is almost flat wrt b
    Vbb = max(Vbb,10^(-6));
    
     %consumption and savings with forward difference
     cf(:,:,1) = Vbf(:,:,1).^(-1/s(1)); % from FOC
     cf(:,:,2) = Vbf(:,:,2).^(-1/s(2));
     sf = zzz + Rb.*bbb - cf;
...
for n=1:maxit
    ...
    cf(:,:,2) = Vbf(:,:,2).^(-1/s(2));
    sf = zzz + Rb.*bbb - cf;
    Hf(:,:,1) = cf(:,:,1).^(1-s(1))/(1-s(1)) +Vbf(:,:,1).*sf(:,:,1);
    Hf(:,:,2) = cf(:,:,2).^(1-s(2))/(1-s(2)) +Vbf(:,:,2).*sf(:,:,2);
    
    %consumption and savings with backward difference
    cb(:,:,1) = Vbb(:,:,1).^(-1/s(1));
    cb(:,:,2) = Vbb(:,:,2).^(-1/s(2));
    sb = zzz + Rb.*bbb - cb;
    Hb(:,:,1) = cb(:,:,1).^(1-s(1))/(1-s(1)) +Vbb(:,:,1).*sb(:,:,1);
    Hb(:,:,2) = cb(:,:,2).^(1-s(2))/(1-s(2)) +Vbb(:,:,2).*sb(:,:,2);
